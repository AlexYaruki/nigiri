...
namespace nigiri
{
	namespace internal
	{
        ...
        JVMMethod::JVMMethod(FR_Id id, jmethodID method): runtimeId(id), method(method) {}
		...
		std::shared_ptr<FR_Method> JVMForeignRuntime::lookupMethod(std::shared_ptr<FR_Type> targetType,
            ...
            execute(op,params);
            return std::make_shared<JVMMethod>(getId(),params->method);
		}
		...
		std::shared_ptr<nigiri::FR_Object> JVMForeignRuntime::call(std::shared_ptr<FR_Type> targetType,
			...
			auto op = [](JNIEnv* env,std::shared_ptr<JVMOpParams> opParams){
				...
				jlong result = env->CallStaticLongMethod(jvmType->getType(),
										                 jvmMethod->getMethod(),
									                     nullptr);
